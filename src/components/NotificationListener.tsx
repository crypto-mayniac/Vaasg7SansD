...
const NotificationListener = () => {
    ...
    useEffect(() => {
        ...
        if (currentNotification) {
            ...
            document.title = `Santa's Daughter - SOL`; // Default title when no active notification
        }
    }, [currentNotification]);
    ...
    const displayNotification = (data) => {
        ...
        const CustomToast = () => (
            <div style={{ display: 'flex', alignItems: 'center' }}>
                ...
                /> */}
                <span>
                    <strong className="" style={{ color: '#9cf8ff' }}>NEW BUY</strong> {data.solSpent} SOL (~${data.usdValue} USD)
                </span>
            </div>
        );
